...
def build_site(summary_md):
    ...
    # 1. Save this daily report to history
    os.makedirs("history", exist_ok=True)
    history_filename = f"history/{today_str}.html"
    ...
    history_links = "".join([f'<li><a href="{f}">{os.path.basename(f).replace(".html", "")}</a></li>' for f in history_files])
...
def build_rss(summary_md):
    ...
    fe = fg.add_entry()
    fe.id(today_str)
    fe.title(f"AI 新聞摘要 - {today_str}")
    fe.link(href=f"{base_url}history/{today_str}.html")
